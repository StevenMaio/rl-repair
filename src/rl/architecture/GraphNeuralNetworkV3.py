"""
This particular implementation iteratively uses the architecture to compute
a learned representation. That is, final point of the computation has the
form f(f(...f(u)...)).
"""
import torch
...
class GraphNeuralNetworkV3(nn.Module):
    _repeated_iterations: int
    ...
    def __init__(self,
        ...
        self._repeated_iterations = repeated_iterations
    ...
    def forward(self, graph: Graph):
        ...
        for iter_no in range(self._num_iterations):
            var_features, cons_features = self._forward_iteration(graph,
                                                                  ...
                                                                  cons_features,
                                                                  iter_no)
        # The repeated final computations
        for iter_no in range(self._repeated_iterations):
            var_features, cons_features = self._forward_iteration(graph,
                                                                  var_features,
                                                                  cons_features,
