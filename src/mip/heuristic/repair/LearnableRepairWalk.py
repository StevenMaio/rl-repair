...
class LearnableRepairWalk(RepairWalk):
    ...
    _fprl: "FixPropRepairLearn"   # I know that doing this is quite horrendous
    ...
    def _sample_violated_constraint(self, model: "Model") -> "Constraint":
        ...
        for cons in model.constraints:
            if cons.is_violated():
                ...
                features.append(self._fprl._cons_features[cons.id])
        if len(cons_ids) == 0:
            return None
    ...
    def _sample_var_candidate(self, cons, candidates):
        features = []
        cons_features = self._fprl._cons_features[cons.id]
        for var, domain_change, shift_damage in candidates:
            features.append(self._create_shift_candidate_feature(cons_features,
                                                                 var,
                                                                 domain_change,
                                                                 shift_damage))
        features = torch.stack(features)
    ...
    def _create_shift_candidate_feature(self, cons_features, var, domain_change, shift_damage):
        var_features = self._fprl._var_features[var.id]
